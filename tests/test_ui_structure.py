...
class TestUIStructure(unittest.TestCase):
    def test_search_async(self):
        ...
        run_search = next((n for n in player_screen.body if (isinstance(n, ast.FunctionDef) or isinstance(n, ast.AsyncFunctionDef)) and n.name == "run_search"), None)
    ...
    def test_login_oauth_async(self):
        ...
        start_oauth = next((n for n in login_screen.body if isinstance(n, ast.FunctionDef) and n.name == "start_oauth_flow"), None)
